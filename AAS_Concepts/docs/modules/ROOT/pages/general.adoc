...
xref:general.adoc#types-and-instances[Types and Instances] describes some general aspects of handling type and instance assets.

xref:general.adoc#identification-of-elements[Identification of Elements] explains the very important aspects of identification in the context of the Asset Administration Shell.

xref:general.adoc#matching-strategies[Matching Strategies] provides matching strategies for semantic identifiers and references.

xref:general.adoc#submodel-instances-and-templates[Submodel Instances and Templates] explains the difference between Submodel instances and templates.

xref:general.adoc#events[Events] discusses aspects of event handling.
...
====
Note 1: to distinguish asset xref:terms-definitions-and-abbreviations.adoc#term-type-asset[type] and asset xref:terms-definitions-and-abbreviations.adoc#term-instance[instance], the term xref:spec-metamodel/core.adoc#AssetKind[asset kind] is used in this document.
...
Other relationships are not explicitly supported by the metamodel of the Asset Administration Shell, but they can be modelled via the xref:spec-metamodel/submodel-elements.adoc#relationship-element-attributes["RelationshipElement"] SubmodelElement type.
====

====
Note 2: Besides asset types and asset instances there als other kinds of assets (see xref:spec-metamodel/core.adoc#AssetKind[enumeration AssetKind]). 
However, in the following examples we do not consider them. 
====

<<table-life-cycle-phases-and-roles-of-type-and-instance-assets>> gives an overview of the different life cycle phases and the role of type assets and instance assets as well as their relationship in these phases.
...
====
Note 3: the NIST model xref:bibliography.adoc#bib49[[49\]] provides an illustration of the second/third class of relationships.
====
...
====
Note 4: for submodels representing the Bill of Material of a complex asset, the metamodel not only provides the possibility to define relationships (via the SubmodelElement xref:spec-metamodel/submodel-elements.adoc#relationship-element-attributes[RelationshipElement], see above), 
it also explicitly supports the representation of another asset (via the SubmodelElement xref:spec-metamodel/submodel-elements.adoc#entity-attributes[Entity]).
...
An Asset Administration Shell typically either represents a xref:terms-definitions-and-abbreviations.adoc#term-type-asset[type asset] or an xref:terms-definitions-and-abbreviations.adoc#term-instance-asset[instance asset].
...
The attribute "assetKind" indicates whether the Asset Administration Shell (denoted by the ":AAS" UML notation for a class instance of the class "AAS") represents a type asset or an instance asset.
...
====
Note 1: the example is simplified for ease of understanding and only roughly complies with the metamodel as specified in  xref:spec-metamodel/index.adoc[].
...
====
Note 3: please refer to  xref:terms-definitions-and-abbreviations.adoc#terms-and-definitions[Terms and Definitions] for the IEC definition of types and instances.
...
The relationship "derivedFrom" of the two Asset Administration Shells "http://T1.com " and "http://T2.com" with Asset Administration Shell "http://T0215551AA.com" will be explained later.
...
The metamodel for Asset Administration Shells is defined in  xref:spec-metamodel/index.adoc#part1-specification[Specification].


The metamodel of the Asset Administration Shell is suitable for type assets or instance assets.
...
The metamodel itself does not require the existence of mandatory submodels.
...
An Asset Administration Shell type shall be realized based on the metamodel of an Asset Administration Shell as defined in this document.
This metamodel is referred to as "Asset Administration Shell Metamodel".


It is not mandatory to define an Asset Administration Shell type before defining an Asset Administration Shell (instance).
...
image::6-unique-identifier-for-aas-and-asset.jpeg[pdfwidth=20%,scaledwidth=20%]
...
See also DIN EN IEC 61406 xref:bibliography.adoc#bib31[[31\]] for further information on identification.
...
<<table-elements-with-allowed-identifying-values>> therefore gives an overview on the different constraints and recommendations on the various entities, which implement xref:spec-metamodel/common.adoc#identifiable-attributes[Identifiable] or xref:spec-metamodel/common.adoc#has-semantics-attributes[HasSemantics].

See Annex xref:concepts-aas.adoc#how-are-new-identifiers-created[How Are New Identifiers Created?] for more information on how to create new identifiers and best practices for creating URI identifiers.
...
| |idShort |string |optionalfootnote:[Note: in version V1.0 of this specification, idShort was optional for identifiables. 
This changed in V2.0: idShort was set to mandatory for all referables. 
With V3.0, idShort was again made optional.]
...
In case of AssetInformation/assetKind = Instance, specify the AssetInformation/globalAssetId of the type asset
...
| |xref:ROOT:spec-metamodel/common.adoc#HasSemantics[semanticId]  |IRDI, IRI (URI) a|
recommended

The semantic ID might refer to an external semantic model defining the semantics of the submodel.

| |xref:ROOT:spec-metamodel/common.adoc#HasSemantics[supplementalSemanticId]  |IRDI, IRI (URI) |optional
...
mandatory with one exception: SubmodelElements within a xref:spec-metamodel/core.adoc#SubmodelElement[SubmodelElementListList] do not need to have an _idShort_ but it is recommended that they do
...
| |xref:ROOT:spec-metamodel/common.adoc#HasSemantics[semanticId]  |IRDI, IRI (URI), Custom a|
recommended
...
|xref:spec-metamodel/common.adoc#Qualifier[Qualifier] |xref:ROOT:spec-metamodel/common.adoc#HasSemantics[semanticId]  |IRDI, IRI (URI), Custom a|
recommended
...
A so-called xref:ROOT:spec-metamodel/common.adoc#HasSemantics[semanticId] should be defined for this Submodel as well as the SubmodelElement.
This semantic ID helps to easily find the semantic definition of the Submodel (see  xref:spec-metamodel/common.adoc#has-semantics-attributes[Has Semantics Attributes]).
...
** Example: xref:ROOT:spec-metamodel/submodel-elements.adoc#Property[Property] with xref:ROOT:spec-metamodel/common.adoc#Referable[idShort] "ManufacturerName" + xref:ROOT:spec-metamodel/common.adoc#HasSemantics[semanticId] value "0173-1#02-AAO677#002"
and Property with idShort "Herstellername" + semanticId value "0173-1#02-AAO677#002" have exactly equal semantics 
if the xref:ROOT:spec-metamodel/referencing.adoc#Key[Key/type]s in both semanticIds are identical 
plus the other attributes of a xref:ROOT:spec-metamodel/referencing.adoc#Reference[Reference] 
like xref:ROOT:spec-metamodel/referencing.adoc#Reference[type] and xref:ROOT:spec-metamodel/referencing.adoc#Reference[referredSemanticId] as well. 
So if the Reference types differ or if the first key type of property "ManufacturerName" is xref:ROOT:spec-metamodel/referencing.adoc#ReferenceTypes[GlobalReference] 
and the first key type of the semanticId of property  "Herstellername" is xref:ROOT:spec-metamodel/referencing.adoc#ReferenceTypes[ConceptDescription] 
then the two semanticIds are no exact match. Or if the first reference contains a referredSemanticId value and the second does not the two semantic IDs are not exact matches.

====
Note 1: Typically, a xref:ROOT:spec-metamodel/referencing.adoc#Reference[Reference]  for a xref:ROOT:spec-metamodel/common.adoc#HasSemantics[semanticId] does not have a xref:ROOT:spec-metamodel/referencing.adoc#Reference[referredSemanticId]
====
...
Clause <<matching-strategies-for-semantic-identifiers>> has discussed matching strategies for semantic identifiers.
This clause explains matching strategies based on the reference concept (see  xref:spec-metamodel/referencing.adoc#referencing-in-aas[Referencing]) in more detail and covers other kinds of identifying elements.

For example, the string serialization of references as defined in  xref:mappings/mappings.adoc#reference-serialization[Text Serialization of Values of Type "Reference"] is used for easier understanding.
...
* Two xref:ROOT:spec-metamodel/referencing.adoc#Reference[References] are identical if all attributes values are identical.
...
* An external reference A matches an external reference B if all values of all xref:ROOT:spec-metamodel/referencing.adoc#Reference[keys]  are identical.
...
====
Note 6: the key type can be ignored since the fragment keys are always unique (e.g. all idShorts of SubmodelElements in a Submodel or all SubmodelElements in a SubmodelElement list or collection).
====

* An external reference A matches a model reference B and vice versa if all values of all keys are identical.

* The xref:ROOT:spec-metamodel/referencing.adoc#Reference[Reference/type] and xref:ROOT:spec-metamodel/referencing.adoc#Reference[Reference/referredSemanticId]  are ignored for matching.
...
====
Note 8: if the values for attribute xref:ROOT:spec-metamodel/referencing.adoc#Reference[referredSemanticId] of the two references compared are not identical then there is a mismatch between the two that should be resolved.
However, the two are considered to match in the context of discovery.
====

====
Note 9 for model reference to model reference matching or external to external reference matching: if the xref:ROOT:spec-metamodel/referencing.adoc#Key[key types] are not identical although all xref:ROOT:spec-metamodel/referencing.adoc#Key[key values]  follow the correct order of the key chain, then at least one of the references is buggy and a warning may be issued.  
...
If the ID of a Submodel is unique, it is not possible that there are two direct SubmodelElement children with the same name (here: Specification).
It is also not possible that two different versions of the same Submodel are compared here, because we would then assume that the ID also contains the version information (see  xref:spec-metamodel/common.adoc#administrative-information-attributes[Administrative Information Attributes]).
...
====
Note 10: this kind of Submodel matching might occur if a xref:ROOT:spec-metamodel/submodel-elements.adoc#SubmodelElement[SubmodelElement]  of type xref:ROOT:spec-metamodel/submodel-elements.adoc#ReferenceElement[ReferenceElement]  is matched against a query for this element.
It is not allowed to substitute the Submodel references within xref:ROOT:spec-metamodel/core.adoc#AssetAdministrationShell[AssetAdministrationShell/submodels]  with an external reference!
====
...
The provider of the Administration Shell can form in-house identifiers for the type and instance of the Submodel in line with  xref:general.adoc#identification-of-elements[Identification of Elements].
...
A public specification of a Submodel template (e.g. via publication by IDTA) should be available to instantiate an existing Submodel template.
...
The same applies to other subtypes of SubmodelElements.
...
SubmodelElement lists typically contain more than one element: the template contains an exemplary element template; the other elements can be created by copy/paste from this template.

[#events]
include::page$includes/events.adoc[]
